...
SUB drawScrollbar
    ...
    FOR n% = INT(thumbpos!) TO INT(thumbpos! + thumblen!)
        LOCATE PAGE_TOP + n%, SCREEN_WIDTH: PRINT "ш";
    NEXT
END SUB
...
SUB printLine (line$)
    ...
    IF line$ = CHR$(CTL_ESCAPE) + CHR$(CTL_LINE1) THEN
        COLOR YELLOW
        PRINT STRING$(PAGE_WIDTH, "м");
        EXIT SUB

    ELSEIF line$ = CHR$(CTL_ESCAPE) + CHR$(CTL_LINE2) THEN
        COLOR YELLOW
        PRINT STRING$(PAGE_WIDTH, "д");
        EXIT SUB

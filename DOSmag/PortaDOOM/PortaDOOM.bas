...
SUB loadPage (page_name$)
    ...
    IF ASC(page_name$, LEN(page_name$) - 2) = ASC("#") THEN
        ...
        'remove the page number from the name
        file_base$ = LEFT$(page_name$, LEN(page_name$) - 3)
    ...
    ELSE
        ...
        page_number% = 0
        file_base$ = page_name$
        file_path$ = PAGE_DIR + file_base$ + ".dosmag"
    END IF
...
SUB wrapLine (line$, align%)
    ...
    FOR c% = c% TO LEN(line$)
        ...
        SELECT CASE char%
            ...
            CASE CTL_CONTROL
                ...
                word$ = word$ + CHR$(CTL_CONTROL) + CHR$(char%)
    ...
    IF l% + w% + 1 > PAGE_WIDTH THEN
        ...
        IF is_heading` = TRUE THEN
            newline$ = newline$ + CHR$(CTL_CONTROL) + CHR$(CTL_HEADING)
        END IF
        'are we currently in parentheses?
        IF word_paren` = TRUE THEN
            newline$ = newline$ + CHR$(CTL_CONTROL) + CHR$(CTL_PAREN)
        END IF
        'are we currently bold?
        IF word_bold` = TRUE THEN
            newline$ = newline$ + CHR$(CTL_CONTROL) + CHR$(CTL_BOLD)
        END IF
        'are we currently italic?
        IF word_italic` = TRUE THEN
            newline$ = newline$ + CHR$(CTL_CONTROL) + CHR$(CTL_ITALIC)
        END IF
        ...
        IF char% = CTL_CONTROL THEN
            ...
            SELECT CASE char%
                ...
                CASE ELSE
                    ...
                    PRINT CHR$(CTL_CONTROL) + CHR$(char%);
